...
def find_latest_heats(conn):
    ...
    # print(sample_chemistry_data['HEAT_NUMBER'])
...
def main():
    ...
    # print('run heats',run_heats)
    # missing_heats=pd.read_excel('missing_heats.xlsx')
    ...
    des_output = pd.DataFrame(columns=['HEAT_NUMBER','GRADE','GRADE_TYPE','AGGREGATE','STATION','AL_chem_first','S_chem_first','C_chem_first','SI_chem_first','First_Probe_temp_lf','Al_Bar_lf','LM_Start_Wt_lf','LTA_lf','Lime_tap','Simn_tap','final_O2_ppm','LOI_perc','AL_KGS','AL_MTS','Carbon_Node','Oxygen_Node'])


    for heats in missing_heats['Heats'].unique():
        print(heats)
        heat_analysis = pd.read_sql("select * from heat_analysis where HEAT_NUMBER in ('22401066','22401166','22301152') order by MSG_TIME_STAMP desc", conn)

        heat_analysis =pd.read_sql(f"SELECT * FROM heat_analysis WHERE HEAT_NUMBER = '{heats}' order by MSG_TIME_STAMP desc ", conn)
        ...
        lf_heat_data = pd.read_sql("select * from lf_heat_data where HEAT_NUMBER in ('22401066','22401166','22301152') order by MSG_TIME_STAMP desc", conn)
        lf_heat_data = pd.read_sql(f"select * from lf_heat_data where HEAT_NUMBER = '{heats}' order by MSG_TIME_STAMP desc", conn)
        ...
        prev_grade_flag=prev_grade_flag.merge(lf_heat_data[['HEAT_NUMBER','GRADE_TYPE']].rename(columns={'HEAT_NUMBER':'PREV_HEAT_NUMBER'}),on=['PREV_HEAT_NUMBER'], how='left')
        ...
        if len(lf_heat_data)>0:
            ...
            print(heat_analysis['HEAT_NUMBER'])
            print(lf_heat_data['HEAT_NUMBER'])
            ...
            if len(ads) > 0:
                ...
                des_output=des_output.append(ads)
        ...
        # conn.commit()
        # cursor.close()
    des_output.to_excel('output.xlsx')
